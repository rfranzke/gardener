...
func (r *Reconciler) reconcileShoot(ctx context.Context, log logr.Logger, shoot *gardencorev1beta1.Shoot) (reconcile.Result, error) {
	...
	if err := r.runReconcileShootFlow(ctx, o, operationType); err != nil {
		flowErr := v1beta1helper.NewWrappedLastErrors(v1beta1helper.FormatLastErrDescription(err), err)
...
func (r *Reconciler) initializeOperation(
	...
	*operation.Operation,
	...
	shootObj, err := shootpkg.
		...
		Build(ctx, r.SeedClientSet, r.GardenClient)
	if err != nil {
		return nil, err
	}
